...
import RenderNLLBids from './RenderNLLBids';
...
function NLLPhunks() {
    ...
    const Fetching = async () => {
         axios.get('http://localhost:8080/api/phunks/bids',{
    ...
    function handlePriceSort(event) {
        if (event.target.value === 'low-to-high') {
            nLLMetaData.sort(function (x, y) {
                return x.bidValue - y.bidValue;
            });
            setPriceQ(event.target.value)
        } else {
            nLLMetaData.sort(function (x, y) {
                return y.bidValue - x.bidValue;
            });
            setPriceQ(event.target.value)
        }
    }
                    ...
                    <div className="row" style={{ justifyContent: 'center' }}>
                        ...
                        <div className="col-md-3 col-5">
                            ...
                            <select className="form-select" aria-label="Price" value={priceQ} onChange={handleFilterPriceSort}>
                                <option value="high-to-low">Highest Bid Phirst</option>
                                <option value="low-to-high">Lowest Bid Phirst</option>
                            </select>
                        </div>
                        <div className="col-md-3 col-5">
